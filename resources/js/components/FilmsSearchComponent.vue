    ...
    <div class="columns is-multiline">
      <div class="card blog-card column is-half is-offset-one-quarter">
        <header class="card-header">
          <h1 class="card-header-title is-size-4 is-centered" v-text="'Search Films'" />
        </header>
        <div class="card-content">
          <div class="content">
            ...
            <table>
              ...
              <tbody>
                <tr v-for="row in filteredFilms" :key="row.id">
                  <td v-for="col in columnKeys" :key="col.id">
                    <template v-if="col==='name'">
                      <a :href="'/film/' + row.slug">
                        <i></i>
                        &nbsp;{{ row[col] }}
                      </a>
                    </template>
  ...
  computed: {
    filteredFilms: function() {
      return this.films.filter(film => {
        return film.name.match(this.search);
      });
    }
  }
};
